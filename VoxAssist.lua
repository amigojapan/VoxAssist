...
--commands
--so list programs, asnwer question, make folder, delete file, delete folder, list files, edit file, I think those are all the menu options I need?
...
while quit==false do
	...
	for index, menu_item_split in ipairs(menu_items_split) do
		...
		--consider comparing even if the condition udnerneeth is not
		--attained, maybe cut on #menu_item_split
		--if (#substrings_of_command >= #menu_item_split) then
		param=findAfterIndex(command_input,#menu_item_split)
		table.insert(parameters, param)
		print("param:"..param)
		joined_menu_item = joinTable(menu_item_split, #menu_item_split)
		difference_in_length=  #substrings_of_command -#menu_item_split
		if(difference_in_length<0) then
			trimmed_command = joinTable(substrings_of_command, #substrings_of_command)
		else	
			--print("1difference_in_length:".. difference_in_length .. "#substrings_of_command:" .. #substrings_of_command .. "#menu_item_split:" .. #menu_item_split)
			number_of_commands=#menu_item_split - difference_in_length
			--print("2number_of_commands:".. number_of_commands .. "#substrings_of_command:" .. #substrings_of_command .. "#menu_item_split:" .. #menu_item_split)
			trimmed_command = joinTable(substrings_of_command, number_of_commands)
		end
		--found=true
		...
		print("comparing "..joined_menu_item.." to " ..trimmed_command .." percent equal...")

		print(" equality ".. math.floor(percentage))
	...
	print("key:" ..  key)
	if maxval < 85 then
		speakAndPrint("sorry I did not catch what you said well")
	elseif key == 1 then
		help(menutable)
	elseif key == 2 then
		run_program(parameters[2])
	elseif key == 3 then
		os.exit()
	elseif key == 4 then
		sayDateAndTIme()
