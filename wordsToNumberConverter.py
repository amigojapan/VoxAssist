#Usage:
#echo "my favorite number is two hundred and fifty three point two , my favorite single numbers are single numbers start one two three end single numbers how about yours?" | python3 wordsToNumberConverter.py
#todo:
#make it so that you can just speak any number and it will be convented to numbers
#del with isngle number like "one"
#move or get rid of simbols after the last number word
...
stringPhrase=""
with open("./tmp/interface.txt", "r") as file:
    # Read the entire content of the file
    #stringPhrase = file.read()
    stringPhrase = file.readline()
...
def conv(stringPhrase):
	...
	if startIndex == None and endIndex == None:#single number may be the only word, replace it with number
			return None, None, None, False, replace_words_with_numbers(stringPhrase)
	if startIndex == None:
			return None, None, None, False, stringPhrase
	startIndex=startIndex-2
	if endIndex == None:
		endIndex=len(stringPhrase)
	else:
		endIndex=endIndex-1
...
while numberDetected:
	...
	#print("wordIndex:"+str(wordIndex)+"startIndex:"+str(startIndex)+"endIndex"+str(endIndex)+"res:"+res)
	stringPhrase=res
	numberDetected=False#added now
